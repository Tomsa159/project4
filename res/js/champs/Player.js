export class Player {
  ...
  draw(ctx) {
    ...
    ctx.save(); //uloží současný stav štětce
      ctx.fillStyle = "red";
      ctx.fillRect(
        (this.position.x - this.size.width / 2)-12,
    ...
    ctx.restore();//vratí stav do předchozího uloženého stavu
  }
  update(keys) {
    this.movement(keys);
    this.attack(keys);
  ...
  movement(keys) {
    if (keys["KeyA"]) {
      //na tomto radku se zastavi kod, tim padame se neprovede
